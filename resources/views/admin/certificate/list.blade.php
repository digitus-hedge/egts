...
<div class="toolbar">
    <form method="GET" action="{{ route('admin.home.certificates') }}" class="search-form">
    ...
    <form method="GET" action="{{ route('admin.home.certificates') }}" class="per-page-form" id="perPageForm">
        @if ($search)
            ...
            <tr>
                <td>
                    @if ($certificate->image)
                        <img src="{{ Storage::url($certificate->image) }}" class="thumb">
                    @else
                        <span class="no-media"><i class="bi bi-image"></i></span>
                    @endif
                </td>
                <td class="title-cell">{{ $certificate->title }}</td>
                <td class="desc-cell">{{ Str::limit($certificate->description, 70) ?: '—' }}</td>
                <td class="text-right">
                    <div class="action-icons">
                        <a href="{{ route('admin.home.certificates.edit', $certificate->id) }}" class="icon-btn icon-edit" title="Edit">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <form action="{{ route('admin.home.certificates.destroy', $certificate->id) }}" method="POST"
                            class="delete-form" id="delete-form-{{ $certificate->id }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
...
<style>
    ...
    .no-media { width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; background: #f4f6f9; border-radius: 6px; color: #bbb; }
