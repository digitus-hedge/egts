...
<div class="wrap">
    ...
    <div class="page-header">
        <div>
            <h1>Certificates</h1>
            <p>The certifications and accreditations shown on your site.</p>
        </div>
        <a href="{{ route('admin.home.certificates.create') }}" class="btn-primary">
            <i class="bi bi-plus-lg"></i> Add New Certificate
        </a>
    </div>
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" action="{{ route('admin.home.certificates') }}" class="search-box" id="searchForm">
                ...
                @if ($search)
                    <a href="{{ route('admin.home.certificates') }}" class="clear-search" title="Clear search">
                        <i class="bi bi-x-circle"></i>
                    </a>
                @endif
            </form>

            <form method="GET" action="{{ route('admin.home.certificates') }}" class="entries-select" id="perPageForm">
                @if ($search)
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions-cell">
                                ...
                                <form action="{{ route('admin.home.certificates.destroy', $certificate->id) }}" method="POST"
                                      id="delete-form-{{ $certificate->id }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="icon-btn delete btn-delete-trigger"
                                            data-form-id="delete-form-{{ $certificate->id }}"
                                            data-name="{{ $certificate->title }}" title="Delete">
        ...
        <div class="empty-state">
            ...
            <p>Try a different search term, or add a new certificate.</p>
        </div>
        @else
        <div class="table-footer">
            <span class="count">Showing {{ $certificates->firstItem() ?? 0 }} to {{ $certificates->lastItem() ?? 0 }} of {{ $certificates->total() }} entries</span>
...
<style>
    ...
    .thumb-wrap{
        width:50px; height:50px; border-radius:10px; overflow:hidden; border:1px solid var(--line,#E9EBF2);
