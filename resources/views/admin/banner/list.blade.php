...
<div class="list-header">
    <h4>All Banners</h4>
    <a href="{{ route('admin.home.banner.create') }}" class="btn-primary">
        <i class="bi bi-plus-lg"></i> Add New Banner
    </a>
</div>

<div class="toolbar">
    <form method="GET" action="{{ route('admin.home.banner') }}" class="search-form">
        <div class="search-input">
            <i class="bi bi-search"></i>
            <input type="text" name="search" value="{{ $search }}" placeholder="Search by title or description...">
        </div>
        @if ($search)
        <a href="{{ route('admin.home.banner') }}" class="clear-search" title="Clear search">
    ...
    <form method="GET" action="{{ route('admin.home.banner') }}" class="per-page-form" id="perPageForm">
        @if ($search)
            ...
            <tr>
                <td>
                    <div class="thumb-group">
                        @forelse ([$banner->image_1, $banner->image_2, $banner->image_3] as $img)
                        @if ($img)
                        <img src="{{ Storage::url($img) }}" class="thumb">
                        @endif
                        @empty
                        @endforelse
                        @if (!$banner->image_1 && !$banner->image_2 && !$banner->image_3)
                        <span class="no-media"><i class="bi bi-image"></i></span>
                        @endif
                    </div>
                </td>
                <td class="title-cell">{{ $banner->title }}</td>
                <td class="desc-cell">{{ Str::limit($banner->description, 60) ?: '—' }}</td>
                <td>
                    @if ($banner->video)
                    <span class="badge badge-success"><i class="bi bi-camera-video"></i> Uploaded</span>
                    @else
                ...
                <td class="text-right">
                    <div class="action-icons">
                        <a href="{{ route('admin.home.banner.edit', $banner->id) }}" class="icon-btn icon-edit" title="Edit">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <form action="{{ route('admin.home.banner.destroy', $banner->id) }}" method="POST"
                            class="delete-form" id="delete-form-{{ $banner->id }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="icon-btn icon-delete btn-delete-trigger"
                                data-form-id="delete-form-{{ $banner->id }}"
                                data-name="{{ $banner->title }}" title="Delete">
