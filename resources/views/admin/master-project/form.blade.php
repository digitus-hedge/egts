...
@section('title', $masterProject->exists ? 'Edit Project' : 'Add Project')
@section('content')
...
<div class="wrap">
    <div class="crumbs">
        <span onclick="window.location='{{ route('admin.home.masters.projects') }}'">Projects</span>
        <span>&rsaquo;</span>
        <b>{{ $masterProject->exists ? 'Edit' : 'Add New' }}</b>
    </div>

    <div class="header">
        <div>
            <h1>{{ $masterProject->exists ? 'Edit Project' : 'Add Project' }}</h1>
            <p>A project entry in the master projects list.</p>
        </div>
        <a href="{{ route('admin.home.masters.projects') }}" class="btn-back">
            <i class="bi bi-arrow-left"></i> Back to list
        </a>
    </div>

    <form action="{{ $masterProject->exists ? route('admin.home.masters.projects.update', $masterProject->id) : route('admin.home.masters.projects.store') }}"
          method="POST" enctype="multipart/form-data" id="masterProjectForm">
        @csrf
        ...
        <div class="card">
            ...
            <div class="field" style="margin-bottom:0;">
                <input type="text" name="title" value="{{ old('title', $masterProject->title) }}"
                       ...
                       placeholder="e.g. Rig Inspection Program">
                @error('title')
        ...
        <div class="card">
            ...
            <div class="field" style="margin-bottom:0;">
                <textarea name="description" rows="4"
                          ...
                          placeholder="Enter project description">{{ old('description', $masterProject->description) }}</textarea>
                @error('description')
                    <span class="field-error"><i class="bi bi-exclamation-circle"></i> {{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="card">
            ...
            <div class="image-slot" style="max-width:300px;">
                <div class="drop img-slot {{ $masterProject->image ? 'filled' : '' }}"      id="drop-master-project-image"  data-file-input="file-image" onclick="handleDropClick(this)">
                    @if ($masterProject->image)
                        <img src="{{ Storage::url($masterProject->image) }}" id="preview-image" alt="Project image">
        ...
        <div class="savebar">
            <div class="savebar-inner">
                <span class="savebar-status">All changes save to the live Projects list</span>
                <div class="btn-group">
                    <a href="{{ route('admin.home.masters.projects') }}" class="btn-cancel">Cancel</a>
                    <button type="submit" class="btn-save">
                        <i class="bi bi-check-lg"></i>
                        {{ $masterProject->exists ? 'Update' : 'Save' }}
                    </button>
                </div>
            </div>
        </div>
...
<script>
document.getElementById('masterProjectForm').addEventListener('submit', function (e) {
    e.preventDefault();
    submitMasterProjectForm();
});
...
function submitMasterProjectForm() {
    ...
    .then(async (response) => {
        ...
        if (response.status === 422 && data && data.errors) {
            showMasterProjectValidationErrors(data.errors);
            return;
        }
        ...
        Swal.fire({
            ...
            window.location.href = (data && data.redirect) ? data.redirect : "{{ route('admin.home.masters.projects') }}";
        });
    })
    .catch(() => {
        Swal.fire({
...
function showMasterProjectValidationErrors(errors) {
    const form = document.getElementById('masterProjectForm');

    const fieldMap = {
        title: f => f.querySelector('[name="title"]'),
        description: f => f.querySelector('[name="description"]'),
        image: f => document.getElementById('drop-master-project-image'),
    };
