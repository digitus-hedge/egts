...
<div class="wrap">
    ...
    <div class="page-header">
        <div>
            <h1>Projects</h1>
            <p>The master list of projects shown across your site.</p>
        </div>
        <a href="{{ route('admin.home.masters.projects.create') }}" class="btn-primary">
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" action="{{ route('admin.home.masters.projects') }}" class="search-box" id="searchForm">
                ...
                @if ($search)
                    <a href="{{ route('admin.home.masters.projects') }}" class="clear-search" title="Clear search">
                        <i class="bi bi-x-circle"></i>
                    </a>
                @endif
            </form>

            <form method="GET" action="{{ route('admin.home.masters.projects') }}" class="entries-select" id="perPageForm">
                @if ($search)
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    @forelse ($masterProjects as $project)
                    <tr>
                        <td>
                        ...
                        <td class="desc-cell">
                            <div class="desc-clamp">{{ Str::limit($project->description, 90) ?: '—' }}</div>
                        </td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('admin.home.masters.projects.edit', $project->id) }}" class="icon-btn edit" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.home.masters.projects.destroy', $project->id) }}" method="POST"
                                      id="delete-form-{{ $project->id }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="icon-btn delete btn-delete-trigger"
                                            data-form-id="delete-form-{{ $project->id }}"
                                            data-name="{{ $project->title }}" title="Delete">
        ...
        <div class="table-footer">
            <span class="count">Showing {{ $masterProjects->firstItem() ?? 0 }} to {{ $masterProjects->lastItem() ?? 0 }} of {{ $masterProjects->total() }} entries</span>
            <div class="pagination-wrap">
                {{ $masterProjects->links() }}
            </div>
        </div>
        @endif
    </div>
