...
<div class="toolbar">
    <form method="GET" action="{{ route('admin.home.masters.projects') }}" class="search-form">
    ...
    <form method="GET" action="{{ route('admin.home.masters.projects') }}" class="per-page-form" id="perPageForm">
        @if ($search)
    ...
    <table class="styled-table">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    @if ($project->image)
                        <img src="{{ Storage::url($project->image) }}" class="thumb">
                    @else
                        <span class="no-media"><i class="bi bi-image"></i></span>
                    @endif
                </td>
                <td class="title-cell">{{ $project->title }}</td>
                <td class="desc-cell">{{ Str::limit($project->description, 60) ?: '—' }}</td>
                <td class="text-right">
                    <div class="action-icons">
                        <a href="{{ route('admin.home.masters.projects.edit', $project->id) }}" class="icon-btn icon-edit" title="Edit">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <form action="{{ route('admin.home.masters.projects.destroy', $project->id) }}" method="POST"
                            class="delete-form" id="delete-form-{{ $project->id }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
...
<div class="pagination-wrapper">
    <div class="pagination-info">
        Showing {{ $masterProjects->firstItem() ?? 0 }} to {{ $masterProjects->lastItem() ?? 0 }} of {{ $masterProjects->total() }} entries
    </div>
