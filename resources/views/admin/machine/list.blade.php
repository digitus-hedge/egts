...
<div class="wrap">
    ...
    <div class="page-header">
        <div>
            <h1>Machines</h1>
            <p>The machines shown on your Facility &amp; Capabilities page.</p>
        </div>
        <a href="{{ route('admin.home.facility.machines.create') }}" class="btn-primary">
            <i class="bi bi-plus-lg"></i> Add New Machine
        </a>
    </div>

    <!-- <div class="stats-row">
        <div class="stat-chip">
            <div class="ico orange"><i class="bi bi-cpu"></i></div>
            <div>
                <div class="num">{{ $machines->total() }}</div>
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" action="{{ route('admin.home.facility.machines') }}" class="search-box" id="searchForm">
                <i class="bi bi-search"></i>
                <input type="text" name="search" id="searchInput" value="{{ $search }}" placeholder="Search by title or description..." autocomplete="off">
                <input type="hidden" name="per_page" value="{{ $perPage }}">
                @if ($search)
                    <a href="{{ route('admin.home.facility.machines') }}" class="clear-search" title="Clear search">
                        <i class="bi bi-x-circle"></i>
                    </a>
                @endif
            </form>

            <form method="GET" action="{{ route('admin.home.facility.machines') }}" class="entries-select" id="perPageForm">
                @if ($search)
                    ...
                    <tr>
                        <td>
                            <div class="thumb-wrap">
                                @if ($machine->image)
                                    <img src="{{ Storage::url($machine->image) }}" alt="{{ $machine->title }}">
                                @else
                                    <i class="bi bi-image" style="color:var(--faint,#9AA1B2);"></i>
                                @endif
                            </div>
                        </td>
                        <td class="title-cell">{{ $machine->title }}</td>
                        <td class="desc-cell">
                            <div class="desc-clamp">{{ Str::limit($machine->description, 90) ?: '—' }}</div>
                        </td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('admin.home.facility.machines.edit', $machine->id) }}" class="icon-btn edit" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.home.facility.machines.destroy', $machine->id) }}" method="POST"
                                      id="delete-form-{{ $machine->id }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="icon-btn delete btn-delete-trigger"
                                            data-form-id="delete-form-{{ $machine->id }}"
                                            data-name="{{ $machine->title }}" title="Delete">
        ...
        <div class="empty-state">
            ...
            <p>Try a different search term, or add a new machine.</p>
        </div>
        @else
        <div class="table-footer">
            <span class="count">Showing {{ $machines->firstItem() ?? 0 }} to {{ $machines->lastItem() ?? 0 }} of {{ $machines->total() }} entries</span>
