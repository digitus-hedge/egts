...
<div class="toolbar">
    <form method="GET" action="{{ route('admin.home.facility.machines') }}" class="search-form">
    ...
    <form method="GET" action="{{ route('admin.home.facility.machines') }}" class="per-page-form" id="perPageForm">
        @if ($search)
    ...
    <table class="styled-table">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    @if ($machine->image)
                        <img src="{{ Storage::url($machine->image) }}" class="thumb">
                    @else
                        <span class="no-media"><i class="bi bi-image"></i></span>
                    @endif
                </td>
                <td class="title-cell">{{ $machine->title }}</td>
                <td class="desc-cell">{{ Str::limit($machine->description, 70) ?: '—' }}</td>
                <td class="text-right">
                    <div class="action-icons">
                        <a href="{{ route('admin.home.facility.machines.edit', $machine->id) }}" class="icon-btn icon-edit" title="Edit">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <form action="{{ route('admin.home.facility.machines.destroy', $machine->id) }}" method="POST"
                            class="delete-form" id="delete-form-{{ $machine->id }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
