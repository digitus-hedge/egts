...
<div class="list-header">
    <h4>Stats Section Entries</h4>
    <a href="{{ route('admin.home.stats.create') }}" class="btn-primary">
        <i class="bi bi-plus-lg"></i> Add New Stat
    </a>
</div>

<div class="toolbar">
    <form method="GET" action="{{ route('admin.home.stats') }}" class="search-form">
        <div class="search-input">
            <i class="bi bi-search"></i>
            <input type="text" name="search" value="{{ $search }}" placeholder="Search by label...">
        </div>
        @if ($search)
        <a href="{{ route('admin.home.stats') }}" class="clear-search" title="Clear search">
    ...
    <form method="GET" action="{{ route('admin.home.stats') }}" class="per-page-form" id="perPageForm">
        @if ($search)
    ...
    <table class="styled-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="title-cell">{{ $stat->label }}</td>
                <td class="desc-cell">{{ Str::limit($stat->description, 70) ?: '—' }}</td>
                <td>
                    @if ($stat->status)
                ...
                <td class="text-right">
                    <div class="action-icons">
                        <a href="{{ route('admin.home.stats.edit', $stat->id) }}" class="icon-btn icon-edit" title="Edit">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <form action="{{ route('admin.home.stats.destroy', $stat->id) }}" method="POST"
                            class="delete-form" id="delete-form-{{ $stat->id }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="icon-btn icon-delete btn-delete-trigger"
                                data-form-id="delete-form-{{ $stat->id }}"
                                data-name="{{ $stat->label }}" title="Delete">
...
<div class="pagination-wrapper">
    <div class="pagination-info">
        Showing {{ $stats->firstItem() ?? 0 }} to {{ $stats->lastItem() ?? 0 }} of {{ $stats->total() }} entries
    </div>
    {{ $stats->links() }}
</div>
...
<style>
    ...
    .value-cell { font-weight: 700; color: #3b3b58; }
