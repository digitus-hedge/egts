...
<div class="wrap">
    ...
    <div class="page-header">
        <div>
            <h1>Tools</h1>
            <p>The tools shown on your Facility &amp; Capabilities page.</p>
        </div>
    ...
    <div class="card">
        <div class="toolbar">
            ...
            <form method="GET" action="{{ route('admin.home.facility.tools') }}" class="entries-select" id="perPageForm">
                @if ($search)
                    ...
                    <tr>
                        <td>
                            <div class="thumb-wrap">
                                @if ($tool->image)
                                    <img src="{{ Storage::url($tool->image) }}" alt="{{ $tool->title }}">
                                @else
                        ...
                        <td class="desc-cell">
                            <div class="desc-clamp">{{ Str::limit($tool->description, 90) ?: '—' }}</div>
                        </td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('admin.home.facility.tools.edit', $tool->id) }}" class="icon-btn edit" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.home.facility.tools.destroy', $tool->id) }}" method="POST"
                                      id="delete-form-{{ $tool->id }}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="empty-state">
            ...
            <p>Try a different search term, or add a new tool.</p>
        </div>
        @else
        <div class="table-footer">
            <span class="count">Showing {{ $tools->firstItem() ?? 0 }} to {{ $tools->lastItem() ?? 0 }} of {{ $tools->total() }} entries</span>
