...
<div class="wrap">
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" action="{{ route('admin.home.projects') }}" class="search-box" id="searchForm">
                <i class="bi bi-search"></i>
                <input type="text" name="search" id="searchInput" value="{{ $search }}" placeholder="Search by title, client, or description..." autocomplete="off">
                <input type="hidden" name="per_page" value="{{ $perPage }}">
                @if ($search)
                    <a href="{{ route('admin.home.projects') }}" class="clear-search" title="Clear search">
                        <i class="bi bi-x-circle"></i>
                    </a>
                @endif
            </form>

            <form method="GET" action="{{ route('admin.home.projects') }}" class="entries-select" id="perPageForm">
                @if ($search)
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('admin.home.projects.edit', $project->id) }}" class="icon-btn edit" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.home.projects.destroy', $project->id) }}" method="POST"
        ...
        <div class="table-footer">
            <span class="count">Showing {{ $projects->firstItem() ?? 0 }} to {{ $projects->lastItem() ?? 0 }} of {{ $projects->total() }} entries</span>
...
<style>
    ...
    .desc-cell{ font-size:13px; color: var(--muted,#667085); max-width:360px; }
